...
def list_products(filter_data=None):
    ...
    rows = db.execute(f"{PRODUCT_SELECT} ORDER BY p.created_at DESC, p.id DESC").fetchall()
...
def search_terms_from_text(text):
    ...
    domain_terms = [
        "翡翠",
        "手镯", "吊坠", "戒指", "戒面", "平安扣", "珠链", "手串", "无事牌", "耳坠", "挂件",
...
def seed_query_concepts():
    concepts = [
        ...
        ("category", "戒指", "戒指", 52, ["戒指", "指环", "翡翠戒指", "方戒", "蛋面戒", "戒托"], ["戒指", "方形", "蛋面", "马鞍", "随形"]),
        ("category", "戒面", "戒面", 50, ["戒面", "戒指面", "蛋面", "裸石", "戒指主石"], ["戒面", "蛋面", "马鞍", "随形"]),
...
def seed_catalog_products(seller_id, count=PUBLIC_PRODUCT_TARGET):
    categories = [
        ...
        ("戒指", ["方形", "蛋面", "马鞍", "随形"], ["10号", "12号", "14号", "16号", "18号"], 19000),
