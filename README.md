...
| Agent 端 | 买家匹配 Agent、商家发布 Agent、客资跟进 Agent、RAG 检索、排序解释、运行 trace |
| 数据层 | SQLite 商品库、商品检索文档、会话消息、客资、Agent 运行记录 |
...
3. **RAG 召回**：用原文和扩展概念查询 `product_documents`，拿到商品证据。
...
2. `search_product_documents()` 查询 `product_documents`。
3. 返回商品 ID、命中词、分数、片段和商品对象。
4. `match_products` 将 RAG 结果和结构化规则合并排序。

RAG 只提供候选和证据，最终推荐仍由排序 Agent 判断。

## 数据库

本地数据库：`data/jade-agent.sqlite`
...
| `backend/db.py` | SQLite schema、种子数据、商品文档、运行记录 |
