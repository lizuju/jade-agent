...
export async function generateProductImage({ prompt }) {
  const id = randomUUID();
  if (!imageClient) {
    const imageUrl = "/assets/jade-bangle-main.jpg";
    recordImageJob({ id, prompt, status: "fallback", imageUrl, provider: "local-asset" });
    return { id, imageUrl, status: "fallback" };
  }
  ...
  try {
    ...
    recordImageJob({ id, prompt, status: "completed", imageUrl, provider: "openai" });
    return { id, imageUrl, status: "completed" };
  } catch {
    const imageUrl = "/assets/jade-bangle-main.jpg";
    recordImageJob({ id, prompt, status: "fallback", imageUrl, provider: "local-asset" });
    return { id, imageUrl, status: "fallback" };
